 ...
 * 侧栏收起态的品牌图形（BrandMark）也吃这个数组 —— 图形与文字同源，永远同色系。
 *
 * 刻意用手挑的固定色组、而不是 `randomHex()` 抽随机色：
 * 随机色经常抽到相邻的两个低对比色（比如深蓝配深紫），
 * 铺在细笔画字形上会糊成一片；固定色组保证每一组都足够鲜艳、明暗有对比。
 *
 * 全部三色，且组内三色必须互相分得开 —— 品牌图形是三条交错的丝带，
 * 两个色太近会让交叠处糊成一条，看不出「交错」。基于这条规则筛过一轮：
 *   - 前半是 logo 自身三色相（紫/珊瑚/绿）整体旋转得到的变体，
 *     色相间距与明暗结构都跟原 logo 一致，所以每个都天然「像 Colora」；
 *   - 后半是原先手挑的那批（含作者原来就在用的几组），补齐成三色。
 * 旋转里黄橙区那几组（+240/270/300）被淘汰了：最暗那条丝带（原绿，L≈52%）
 * 转到黄橙会变成橄榄/棕，而黄色要够亮才鲜艳，固定明度的色相旋转做不到。
 *
 * 首组就是 logo 原色，等于默认状态下图形与 favicon 完全一致。
 */
const LOGO_GRADIENTS: readonly (readonly string[])[] = [
  // logo 原色
  ["#9D65FB", "#F47581", "#4AC14D"],
  // logo 三色相整体旋转
  ["#E865FB", "#F4A875", "#4AC188"],
  ["#FB65C3", "#F4E875", "#4ABEC1"],
  ["#FB6578", "#C1F475", "#4A83C1"],
  ["#FB9D65", "#81F475", "#4D4AC1"],
  ["#FBE865", "#75F4A8", "#884AC1"],
  ["#C3FB65", "#75F4E8", "#C14ABE"],
  ["#78FB65", "#75C1F4", "#C14A83"],
  // 手挑的跨色相组合
  ["#F97316", "#8B5CF6", "#14B8A6"],
  ["#6366F1", "#EC4899", "#FACC15"],
  ["#14B8A6", "#FACC15", "#6366F1"],
  ["#8B5CF6", "#06B6D4", "#F43F5E"],
  ["#F43F5E", "#FB923C", "#3B82F6"],
  ["#22C55E", "#0EA5E9", "#F472B6"],
  ["#3B82F6", "#8B5CF6", "#EC4899"],
  ["#EF4444", "#F59E0B", "#10B981"],
  ["#0891B2", "#7C3AED", "#FB7185"],
  ["#FB923C", "#8B5CF6", "#22D3EE"],
  ["#A3E635", "#8B5CF6", "#F472B6"],
];
